...
| **Impact** | Bug fixes must be applied twice. Template drift. Inconsistent validation. |
...
### ISSUE-13: `utils.ts` Single-Function Anti-Pattern

| | |
|---|---|
| **File** | `src/lib/utils.ts` (7 lines, single `cn()` function) |
| **Problem** | Clean-code skill says: *"utils.ts with 1 function → Put code where used."* |
...
| **Fix** | Use ES2022 Error `cause` option: `new ApplicationError('...', { cause: error })`. |
...
| **Impact** | High — missing rich snippets (Person, BlogPosting, BreadcrumbList, WebSite). |
