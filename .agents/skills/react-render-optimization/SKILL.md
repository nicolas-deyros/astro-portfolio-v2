...
- Apply these patterns during code generation, review, and refactoring. When you see an antipattern, suggest the corrected version with an explanation.
	...
	return (
		<article>
			...
			<time suppressHydrationWarning>
				...
				{/* "2 minutes ago" differs server vs client */}
			</time>
		</article>
	)
}
...
function ProductLink({ id }: { id: string }) {
	...
	return (
		<Link to={`/products/${id}`} onMouseEnter={handleHover}>
			View
		</Link>
	)
}
...
function NavLink({ to, children }: { to: string; children: React.ReactNode }) {
	...
	return (
		<a href={to} onClick={handleClick} style={{ opacity: isPending ? 0.7 : 1 }}>
			{children}
		</a>
	)
}
