...
In our applications, we often have to create many objects of the same type. A useful way of doing this is by creating multiple instances of an ECMAScript 2015 class.

## When to Use

- Use this when many objects need access to the same methods without duplicating them
- This is helpful for understanding JavaScript's inheritance model and ECMAScript 2015 classes

## Instructions

- Use ECMAScript 2015 classes to automatically add methods to the prototype
- Use `Object.create()` to create objects with a specific prototype
- Leverage the prototype chain for inheritance (`extends` keyword in ECMAScript 2015 classes)
...
Notice here how the `constructor` contains a `name` property, and the class itself contains a `bark` property. When using ECMAScript 2015 classes, all properties that are defined on the class itself, `bark` in this case, are automatically added to the `prototype`.
