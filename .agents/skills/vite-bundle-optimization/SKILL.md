...
Barrel files (`index.ts` that reexport from many modules) force bundlers to load the entire module graph even when you only use one export. This is the #1 bundle size issue in React apps.
...
**Autofix with `vite-plugin-barrel`:**
