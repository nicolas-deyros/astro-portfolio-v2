...
### 8. Hoist regular expression and Constant Creation Outside Loops

**Impact: LOW-MEDIUM** — Avoids repeated compilation.

Creating regular expression objects or constant values inside loops or frequently-called functions wastes CPU.

**Avoid — compiles regular expression 10,000 times:**
...
`structuredClone()` handles circular references, typed arrays, Dates, regular expressions, Maps, and Sets — unlike `JSON.parse(JSON.stringify())`.
