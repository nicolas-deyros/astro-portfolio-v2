...
We have had the unique opportunity to work on performance analysis for many of our partner's sites. We identified multiple similar issues that plagued the efficient loading of pages across different partner sites.
...
3P libraries are often required to add common features and functionality to the site. Third parties include ads, analytics, social widgets, live chat, and other embeds that power a site. A third party library comes with its own JavaScript, images, fonts etc.
...
If inlining is not possible, critical CSS should be preloaded and served from the same origin as the document. Avoid serving critical CSS from multiple domains or direct use of third-party critical CSS like Google Fonts. Your own server could serve as a proxy for third-party critical CSS instead.
...
| Fonts      | Fonts are neither inlined nor do they use preconnect. Fonts are loaded via external style sheets which delays the loading. Fonts may or may not be display blocking. |
